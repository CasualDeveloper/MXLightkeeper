...
2. `refreshReceivers` enumerates exact matches, preferring nonexperimental receivers and then lower location IDs. The first result becomes the active receiver address. Enabled status moves to `.starting`; there is no user-selectable receiver or paired-slot discovery.
3. If enabled, `prepareRuntimeState` resolves the first match again and rejects a snapshot mismatch. `BacklightKeeper.start` acquires ownership and schedules the task.
4. The keeper attempts a refresh immediately, then every 180 seconds. `refreshKeepAlive` writes `offSignal`, waits 50 ms, then writes `onSignal`. Both packets use receiver slot `0x01` and feature index `0x0b`. The result records whether neither report, only the first report, or both reports returned transport success. A complete result moves status to `.active`; failure moves it to `.degraded`.
...
The CLI's `keep` uses the same keeper and exits cleanly on Ctrl-C. It does not run the app's receiver poll, although raw writes re-enumerate the saved snapshot and may resume after a matching reconnection. Its other commands open short-lived sessions: `read` reads structured state; `on`, `off`, and `manual` read, write, then read again. `manual` accepts levels 1 through 7. Structured requests rotate a conservative pool of nonzero software IDs, excluding the raw pulse ID and IDs documented as used by other tools. These operations currently use fixed slot `0x01` and `BACKLIGHT2` index `0x0b`. Name and battery feature indexes are dynamically resolved.

### What current feedback proves

- `isEnabled` is persisted intent. `BacklightKeeper.isRunning` means its task and ownership were established. `.starting` means a receiver address is selected but no complete refresh has been observed.
- `.active` means both reports in the latest observed pulse returned transport success. It proves neither protocol readback nor physical illumination. A partial pulse records `.offReportSent`; a first-report failure records `.notStarted`.
- `.waiting` represents enabled intent without a receiver. `.degraded` represents enumeration, ownership/start, or refresh failure. The release subtitle stays generic because those causes share one broad presentation state; `lastErrorMessage` retains the operation detail but is not rendered in the release panel.
...
- Structured response correlation matches device index plus the transmitted feature and packed function/software-ID bytes. HID++ 2.0 `0xFF` errors decode their echoed request bytes and throw typed failures instead of becoming ordinary responses. Rotating software IDs reduce ambiguity from late replies but cannot prove freshness after pool wraparound or process restart.
- Setters validate only requested readback semantics: enabled state for `on`/`off`, and enabled manual mode plus level for `manual`. They return the observed state. Matching readback still does not prove visible LED behavior.
- `lastRefreshResult` retains the target, attempt time, completed pulse stage, and failure description. `lastErrorMessage` exists in the model but is not rendered in the release panel. OSLog records refresh completion/failure without claiming visible light.
...
- Keeper lifetime still relies on explicit `stop`/`close` for immediate cancellation and ownership release. Its task holds only a weak keeper reference, so dropping the keeper prevents later scheduled writes; normal app stop/quit paths explicitly stop it. Process-level teardown and in-flight native writes remain untested.
...
- The CLI parses the complete invocation before constructing its controller. Unknown flags, surplus arguments, duplicate `--level`, and levels outside 1 through 7 fail before device access. Proposed receiver, JSON, duration, and timeout flags remain unavailable and are rejected.
...
`HIDPPProbeSession` starts its wall-clock response timeout after the synchronous native write; it does not bound enumeration, opening, or that write. Its exact-size parser and correlation fixtures are synthetic contract evidence, not captured hardware traffic.

### Presentation limits

All 31 catalog keys referenced by current Swift literals have eight-language entries. The remaining 11 keys are unreferenced fragments, including old language-picker text. Their presence does not imply a language override. Chinese accessibility percentage formats escape literal percent characters according to [Apple's format contract](https://developer.apple.com/library/archive/documentation/Cocoa/Conceptual/Strings/Articles/formatSpecifiers.html).
...
Alpha suffixes use the same explicit locale/bundle path as their containing strings. Status transitions restore visibility on equal/reduced-motion paths and do not publish after cancelled sleeps. Debug Pulse, Demo, and Probe controls share a model-level overlap guard and are disabled while an action runs. Rendered rapid-transition and conflicting-preference checks remain manual verification gaps.
...
An absent settings record uses the intentional first-install default of enabled keeping. Existing settings decode missing fields independently with fail-closed enabled intent, and malformed existing data is logged and loaded disabled. Encoding preserves both current Boolean fields. Launch-at-login still reflects stored intent rather than startup reconciliation with `SMAppService` status.

[Package.swift](../Package.swift) defines the core, two executables, and three test targets without external package dependencies. [build-app-bundle.sh](../scripts/build-app-bundle.sh) builds a universal release executable, requires and copies the compiled SwiftPM resource bundle, replaces `dist/MXLightkeeper.app`, and ad-hoc signs it. [CI](../.github/workflows/ci.yml) builds and tests, but does not package or validate hardware.

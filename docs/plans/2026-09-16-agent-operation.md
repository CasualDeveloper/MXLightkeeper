...
- `BacklightKeeper.start` drops every refresh error. The reducer can report `.active` on receiver acquisition alone; keeper startup also assigns it without a refresh outcome.
...
- CLI output is text-only and extra arguments are not reliably rejected. A speculative flag can therefore be ignored rather than fail safely.
- Language unit tests cover selection, not catalog completeness or packaged resources. Bundle copying is optional in the packaging script, and CI never packages the app.
- Existing settings decode failures fall back to enabled intent. All used catalog keys cover the supported languages, but two Chinese accessibility formats contain unescaped percent signs. Status animation and alpha-suffix lookup also need targeted runtime checks.
...
1. Validate complete CLI invocations before device access. Reject unknown/proposed flags, surplus arguments, duplicate/conflicting options, and invalid values. Separate parsing from execution so tests can prove zero transport calls on rejected input.
2. Add a compact refresh outcome: target, attempt time, completed stage, error, and whether both reports were accepted. Keep desired mode, last attempt, and last successful transmission separate. Do not label transport acceptance as visible light.
3. Reuse `ReceiverEnumerating`, `ReceiverWriting`, and the keeper's refresh-operation seam. Add only the request/session and scheduling seams needed for named tests. Exercise production pulse orchestration instead of treating the unused structured helper as keeper coverage.
4. Correlate HID++ errors to the request and propagate them as failures. Validate setter readback against the requested enabled/mode/level semantics, not unrelated fields. Add fixtures for unrelated and late replies as well as explicit errors.
5. Make the reducer consume refresh outcomes instead of treating enumeration as recovery. Preserve failure causes through successful polling and keep sample freshness/target association explicit. Distinguish no receiver, busy ownership, and write failure in accessible whole-string feedback. Describe disabled keeping as intent, not measured darkness; keep login-service errors separate from keeper health.
6. Distinguish first installation from damaged existing settings. Preserve valid disabled intent and keep control disabled when existing enabled intent cannot be decoded; log recovery instead of silently resetting to enabled. Retain the intentional first-install default. Reconcile launch-at-login presentation with service status through a narrow app adapter without automatically undoing a user's external disablement.
7. Correct the two Chinese accessibility formats and route alpha suffixes through the same explicit language resolver as their containing strings. Verify actual formatting before claiming an observed symptom. Remove unreferenced catalog fragments only after checking their consumers. Preserve whole-string localization and the accepted layout.
8. Reproduce rapid subtitle transitions, then make cancellation and early-return paths preserve current text and visibility. Test changes to Reduce Motion and view dismissal as well as status changes. Guard overlapping debug actions in the model and disable all mutating debug controls while a sequence runs; shared production/debug transactions follow in slice 2.

**Acceptance:** invalid/speculative flags cause no device access; help works without hardware. Scripted first/second-write failure, recovery, metadata success after write failure, protocol rejection, and mismatched readback report the correct outcome. Advance virtual time for cadence checks. App-model fixtures use isolated defaults and injected platform effects so initialization cannot probe hardware or register login items.
...
2. Make a missing resource bundle a packaging failure immediately, independently of the runtime sequence. Add a bundle-check mode verifying both architectures, metadata, ad-hoc signature, and compiled localizations. Keep the fast lane separate from the universal build.
